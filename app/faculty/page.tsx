...
export default async function FacultyPage() {
  ...
  return (
    <main className="directory-page">
      ...
      <section className="directory-grid">{faculty.map(({ slug, entry }) => <article className="directory-card faculty-card" key={slug}>{entry.photoUrl && <img className="faculty-portrait" src={entry.photoUrl} alt={entry.name} />}<div className="faculty-card-overlay" /><div className="faculty-card-content"><span className="card-index">FACULTY</span><h2>{entry.name}</h2><p className="card-meta">{entry.role}</p><p>Our faculty members connect careful study with faithful ministry and a generous life of learning.</p><Link href="/keystatic/collection/faculty">Edit faculty <span>↗</span></Link></div></article>)}</section>
